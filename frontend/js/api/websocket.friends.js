(function(global) {
  ...
  const FriendWS = {
    ...
    handleMessage(data) {
      ...
      // Dispatch custom event for notification manager
      const customEvent = new CustomEvent('ws-message', { detail: data });
      window.dispatchEvent(customEvent);

      switch (type) {
        case 'NEW_NOTIFICATION':
          // Handled by notification manager via custom event
          break;
  ...
  if (window.api && api.getToken && api.getToken()) {
    setTimeout(() => {
      console.log('Auto-connecting FriendWS...');
      FriendWS.connect().catch(err => {
        console.warn('FriendWS auto-connect failed:', err);
      });
    }, 500);
  }
